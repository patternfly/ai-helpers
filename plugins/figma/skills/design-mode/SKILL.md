...
Before any `use_figma` call, load and follow the `figma-use` Claude skill.
...
3. Resolve components from approved sources
   - Search and import from the two approved PatternFly files only.
   - Verify each chosen component/pattern comes from an approved source.
...
- Reuse approved library components first; avoid duplicate ad-hoc components.
- Bind variables/tokens where possible (color, type, spacing, radius).
- Keep names semantic and stable for handoff (frames, components, variants).
- If requirements are ambiguous, ask a focused clarification question before making large edits.
---
...
1. Confirm target
   - Identify `fileKey` and `nodeId` from the Figma URL when provided.
   - Clarify expected output (new screen, edits to an existing frame, component updates).
...
4. Make incremental edits
   - Prefer editing existing nodes/components over rebuilding from scratch.
   - Use approved design-system components and variables instead of hardcoded values.
   - Apply changes in small batches and verify after each batch.
...
---
name: design-mode
description: Create and edit Figma design files using the Figma MCP workflow. Use when the user asks to build, update, restyle, or restructure Figma frames/components, or provides a figma.com URL and wants write operations.
---
...
3. Inspect before editing
   - Read current document context and structure.
   - Locate existing design-system components and variables before creating new nodes.

4. Make incremental edits
   - Prefer editing existing nodes/components over rebuilding from scratch.
   - Use design-system components and variables instead of hardcoded values.
   - Apply changes in small batches and verify after each batch.

5. Validate outcome
   - Check hierarchy, alignment, spacing, constraints, and variant states.
   - Ensure visual consistency with the design system and requested scope.

## Guardrails

- Reuse library components first; avoid duplicate ad-hoc components.
- Bind variables/tokens where possible (color, type, spacing, radius).
- Keep names semantic and stable for handoff (frames, components, variants).
- If requirements are ambiguous, ask a focused clarification question before making large edits.
---
