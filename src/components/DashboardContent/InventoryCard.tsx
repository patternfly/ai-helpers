...
export const InventoryCard: React.FunctionComponent = () => (
  <Card id="dashboard-demo-data-list-card-1">
    <CardHeader className="pf-v6-u-align-items-flex-start">
      <CardTitle id="dashboard-demo-data-list-card-1-title1">
        Inventory
      </CardTitle>
    </CardHeader>
    <DataList
    ...
    >
      {data.map(({ count, text }, idx) => (
        <DataListItem
          key={`item-${idx}`}
          aria-labelledby={`dashboard-demo-data-list-card-1-data-list-item-${
            idx + 1
          }`}
        >
          <DataListItemRow>
            <DataListItemCells
              dataListCells={[
                <DataListCell
                  key={`${idx}-1`}
                  id={`dashboard-demo-data-list-card-1-data-list-item-${
                    idx + 1
                  }`}
                >
                  {count} {text}
